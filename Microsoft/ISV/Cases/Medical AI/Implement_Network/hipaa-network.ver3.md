...
> **버전**: v3 (2026-04-20 — 리전 미국 변경, 아키텍처 다이어그램 추가)

---

## 목차

1. [아키텍처 다이어그램](#0-아키텍처-다이어그램)
2. [Azure VPN Gateway — 전송 중 암호화](#1-azure-vpn-gateway)
3. [Azure Firewall Premium — 네트워크 전송 보안](#2-azure-firewall-premium)
4. [Private Endpoint — 공인 인터넷 경유 차단](#3-private-endpoint)
5. [TLS 최소 버전 강제 설정](#3-4-tls-최소-버전-강제-설정)
6. [NSG — 서브넷 레벨 접근 통제](#3-5-nsgnetwork-security-group--서브넷-레벨-접근-통제)
7. [요약 — HIPAA 조항별 구현 매핑](#요약--hipaa-조항별-구현-매핑)
...
> Connection 생성 후 → `conn-hospital-us-s2s` → Configuration → Use custom IPsec/IKE policy: **켬**
...
>
> **PSK 관리 정책**: PSK는 Key Vault secret(`vpn-psk-hospital-us`)으로 관리. 교체 주기는 연 1회(또는 직원/계약 변경 시 즉시). 인증서 기반 IKEv2(EAP-TLS) 전환이 권고되나, 온프레미스 VPN 장비 미지원 시 PSK + Key Vault 조합을 허용된 대안으로 채택.
...
| 소분류 | 항목 | 예시 값 |
|---|---|---|
| 기본 정보 | Name | `afw-medicalai-hub-us` |
| | Region | East US |
| | Tier | **Premium** *(TLS 검사 및 IDPS 필수)* |
| 네트워크 | Virtual network | `vnet-hub-us` |
...
### 2-4. Network Rule — 미국 병원 → Azure 허용 경로
...
| 소분류 | 항목 | 예시 값 |
|---|---|---|
| 컬렉션 | Name | `netrc-hospital-us-to-ai` |
| | Priority | `100` |
| | Action | Allow |
| Rule 1 (VPN → AKS) | Name | `allow-hospital-us-vpn-to-aks` |
| | Source | `192.168.10.0/24` |
| | Destination | `10.2.10.0/24` (AI Spoke AKS Subnet) |
| | Protocol | TCP |
| | Destination Port | `443` |
| Rule 2 (VPN → Storage) | Name | `allow-hospital-us-vpn-to-storage` |
| | Source | `192.168.10.0/24` |
...
> `afw-medicalai-hub-us` → Diagnostic settings → + Add
