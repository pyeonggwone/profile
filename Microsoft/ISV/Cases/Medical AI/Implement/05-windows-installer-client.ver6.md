...
> 버전: v6 (2026-04-20 — k3s 컨테이너 방식 신규 작성)

---

## 배포 방식 선택 기준

| 항목 | 이 가이드 (k3s) | [06 가이드 (MSI)](./06-windows-installer-client.ver1.md) |
|---|---|---|
...
| **배포 단위** | 컨테이너 이미지 | MSI 패키지 |
| **원격 업데이트** | Arc GitOps (Flux) | Arc Run Command (msiexec) |
| **병원 IT 요구 역량** | 높음 (K8s 기본 이해 필요) | 낮음 (표준 Windows 운영) |

> **선택 기준**: 병원 서버 OS 확인 후 결정. OS 버전이 불명확하거나 혼재한다면 [06 가이드 (MSI)](./06-windows-installer-client.ver1.md) 사용.
...
| Windows Server 2016 이하 | ❌ 불가 | ✅ | 조건부 | → [06 가이드 (MSI)](./06-windows-installer-client.ver1.md) 사용 |
...
docker build -t pii-masking-service:v6.0 ./src/pii-masking
docker build -t rabbitmq-producer:v6.0 ./src/rabbitmq-producer
docker build -t config-sync:v6.0 ./src/config-sync

# ACR 푸시
az acr login --name acrmedicalai
docker tag pii-masking-service:v6.0 acrmedicalai.azurecr.io/pii-masking-service:v6.0
docker tag rabbitmq-producer:v6.0 acrmedicalai.azurecr.io/rabbitmq-producer:v6.0
docker tag config-sync:v6.0 acrmedicalai.azurecr.io/config-sync:v6.0
docker push acrmedicalai.azurecr.io/pii-masking-service:v6.0
docker push acrmedicalai.azurecr.io/rabbitmq-producer:v6.0
docker push acrmedicalai.azurecr.io/config-sync:v6.0
```

> **주의**: .NET 8 Linux 컨테이너로 빌드. k3s는 Linux 컨테이너를 Windows 노드에서 실행하는 방식이 아닌, k3s Linux 에이전트 노드 방식. 아래 아키텍처 참고.

> **실제 구성**: k3s를 Windows Server에서 직접 실행하는 방식은 현재 k3s 에이전트 노드 한정. Linux 컨테이너를 실행하려면 병원 서버에 WSL2 또는 별도 Linux VM이 필요하거나, k3s를 **Linux OS 병원 서버**에 설치해야 함.
...
> 이 가이드는 **구성 A (Linux 서버)** 기준. 병원 서버가 Windows만 있다면 [06 가이드 (MSI)](./06-windows-installer-client.ver1.md) 권장.
...
```bash
# ISV 개발팀 — 새 이미지 푸시
docker push acrmedicalai.azurecr.io/pii-masking-service:v6.1
