...
**1. 공유 범위 계층 통제 운영 방안** (메인 질문)
- 1-1. 개인/팀 공유는 허용하면서 전사 공유는 관리자 승인 기반으로 통제하는 운영 방안
- 1-2. 광범위 공유 발생 시 관리자 승인 없이는 배포되지 않도록 제한하는 기능 또는 정책

**2. Agent 공유 범위 제한 - 사용자/그룹 수준 정책**
- 2-1. 특정 사용자 개인에게만 공유를 제한하는 설정 방법
- 2-2. 특정 보안 그룹(팀/부서) 단위로 공유 범위를 제한하는 정책 설정 방법

**3. 조직 전체(전사) 공유 제한 및 비활성화 관리**
- 3-1. "조직 내 모든 사용자" 대상 공유를 관리자가 차단하는 방법
- 3-2. 전사 배포 자체를 비활성화하는 전용 관리 설정

**4. 관리자 승인 기반 배포 프로세스 구현**
- 4-1. 네이티브 기능 기반 승인 게이트 구성 방법 (Power Platform 파이프라인)
- 4-2. 권장 아키텍처 (환경 분리 + 승인 게이트 조합)
