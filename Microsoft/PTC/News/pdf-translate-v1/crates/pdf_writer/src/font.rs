...
use pdf_core::{ObjectId, PdfDict, PdfNumber, PdfObject, PdfResult, PdfStream};
...
impl EmbeddedFont {
    ...
    pub fn write(
        ...
        // 1. Subset the font.
        let mut used_gids: BTreeSet<u16> = self
            .used_chars
            .iter()
            .filter_map(|c| self.char_to_gid.get(c).copied())
            .collect();
        used_gids.insert(0); // .notdef
        let gid_list: Vec<u16> = used_gids.iter().copied().collect();
        let subset_bytes = match subsetter::subset(&self.original_bytes, 0, gid_list.iter().copied()) {
            Ok(b) => b,
            Err(_) => self.original_bytes.clone(), // fall back to full font
        };
