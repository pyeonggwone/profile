...
use pdf_writer::{build_image, ContentStreamBuilder, ImageInput, PdfFileBuilder};
...
impl<'a> IncrementalWriter<'a> {
    ...
    pub fn build(&self, operations: &[EditOperation]) -> PdfResult<IncrementalUpdate> {
        ...
        for op in operations {
            if let EditOperation::AddText { font, .. } = op {
        ...
        for (page_index_1, ops) in &by_page {
            ...
            let has_drawing = ops.iter().any(|op| {
                matches!(
                    op,
                    EditOperation::AddText { .. } | EditOperation::AddImageJpeg { .. }
                )
            });
