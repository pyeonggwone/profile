# build.md — pdf-translate-v11 빌드 파이프라인 설계

이 문서는 `pdf-translate-v11` 의 빌드/실행 파이프라인을 정의한다. 여기서 빌드는 소스 컴파일만 의미하지 않고, 원본 PDF를 번역 PDF로 만드는 전체 처리 과정을 의미한다.
...
```bash
./run-v11.sh --in-lang en --out-lang kr input/source.pdf
./run-v11.sh --ocr local input/source.pdf
./run-v11.sh --ocr azure input/source.pdf
./run-v11.sh --translation-mode openai input/source.pdf
./run-v11.sh --translation-mode azure-openai input/source.pdf
./run-v11.sh --keep-work input/source.pdf
```

현재 구현은 `run-v11.sh` 와 `src/pdf_translate_v11/pipeline.py` 로 동작한다. 고품질 목표 도구가 설치되지 않은 경우에는 실패 대신 `degraded` 상태를 기록하고 가능한 adapter로 진행한다. `--strict` 를 사용하면 권장 도구 누락을 실패로 처리한다.

`run-v11.sh` 는 내부적으로 필요한 언어와 도구를 호출한다.
...
v11은 결과 PDF를 만든 뒤 반드시 품질 검증을 수행한다.
...
`run-v11.sh` 는 PDF 처리 로직을 직접 구현하지 않는다. 각 단계의 전용 도구와 script를 순서대로 호출하는 orchestrator 역할만 담당한다.

## 결론

v11 build 구조의 핵심은 다음이다.
