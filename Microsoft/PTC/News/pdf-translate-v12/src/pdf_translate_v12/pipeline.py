...
STEP_ORDER = [
    ...
    "09_build_segments",
    "10_build_source_pdf",
    "11_translate_segments",
    "12_shape_text",
    "13_layout_text",
    "14_build_translated_pdf",
    "15_validate_output_pdf",
    "16_render_diff",
    "17_publish_output",
]
...
class Pipeline:
    ...
    def _ensure_textless_backgrounds(self) -> dict[str, Any]:
        ...
        if textless_pdf.exists() and render_path.exists():
            self._preserve_stage_pdf(1, "base", textless_pdf)
    ...
    def _build_pdf_with_reportlab(self, target: Path, layout_path: Path | None = None) -> dict[str, Any]:
        from reportlab.pdfbase import pdfmetrics  # type: ignore
        from reportlab.pdfbase.ttfonts import TTFont  # type: ignore
        from reportlab.pdfgen import canvas  # type: ignore

        layout = read_json(layout_path or self.state_dir / "positioned-layout.json", {}) or {}
        ...
        font_names: dict[str, str | None] = {
            "regular": self._reportlab_register_font(pdfmetrics, TTFont, "V12Regular", self.config.font_regular) or "Helvetica",
            "bold": self._reportlab_register_font(pdfmetrics, TTFont, "V12Bold", self.config.font_bold) or "Helvetica-Bold",
            "fallback": self._reportlab_register_font(pdfmetrics, TTFont, "V12Fallback", self.config.font_fallback),
        }
    ...
    def _build_pdf_with_cairo(self, target: Path, layout_path: Path | None = None) -> dict[str, Any]:
        cairo, Pango, PangoCairo = self._load_pango_cairo()
        layout_state = read_json(layout_path or self.state_dir / "positioned-layout.json", {}) or {}
    ...
    def _build_pdf_with_pymupdf(self, target: Path, layout_path: Path | None = None) -> dict[str, Any]:
        import fitz  # type: ignore
        layout = read_json(layout_path or self.state_dir / "positioned-layout.json", {}) or {}
    ...
    def _layout_text_items(self, source_items: list[dict[str, Any]], text_key: str, origin: str) -> dict[str, Any]:
        ...
        for source_item in source_items:
            page_number = int(source_item.get("page") or 1)
            page_info = page_sizes.get(page_number, {"page": page_number, "width": source_item.get("pageWidth") or 612.0, "height": source_item.get("pageHeight") or 792.0})
            page_entry = pages.setdefault(page_number, {"page": page_number, "width": page_info["width"], "height": page_info["height"], "items": []})
            bbox = source_item.get("bbox") or {"x": 0, "y": 0, "width": 160, "height": 20}
            style = dict(source_item.get("style") or {"fontSize": 10, "color": [0, 0, 0]})
            style["layoutContext"] = source_item.get("layoutContext", "text-flow")
            text = str(source_item.get(text_key) or source_item.get("source") or "")
            try:
            ...
            layout_item = {
                ...
                **self._font_state(fitted_style, layout_font_size),
                "origin": origin,
                "layoutContext": source_item.get("layoutContext", "text-flow"),
                **({"tableBBox": source_item.get("tableBBox")} if source_item.get("tableBBox") else {}),
    ...
    def _build_text_layer_pdf(self, target: Path, layout_path: Path, step_name: str) -> dict[str, Any]:
        self._ensure_textless_backgrounds()
        if module_available("reportlab"):
            report = self._build_pdf_with_reportlab(target, layout_path)
            report["preferredOver"] = "cairo"
            report["preferredReason"] = "embedded-font-visible-text"
            return report
        try:
            return self._build_pdf_with_cairo(target, layout_path)
        except Exception as exc:
            if not self.config.allow_degraded:
                raise StepError(step_name, f"Cairo PDF builder failed: {exc}")
            if module_available("fitz"):
                report = self._build_pdf_with_pymupdf(target, layout_path)
                report["degradedFrom"] = "cairo"
                report["degradedReason"] = str(exc)
                return report
            raise StepError(step_name, "ReportLab is not installed, Cairo failed, and PyMuPDF degraded builder is disabled")
    ...
    def step_10_build_source_pdf(self) -> None:
        payload = read_json(self.state_dir / "segments.json", {}) or {}
        layout = self._layout_text_items(payload.get("segments") or [], "source", "source-text")
        layout_path = self.state_dir / "source-positioned-layout.json"
        write_json(layout_path, layout)
        self.set_artifact("sourcePositionedLayout", layout_path)

        target = self.pdf_dir / "source-text.pdf"
        report = self._build_text_layer_pdf(target, layout_path, "10_build_source_pdf")
        report_path = self.state_dir / "source-build-report.json"
        write_json(report_path, {"status": "ok", **report, "recordedAt": now_iso()})
        self.set_artifact("sourceTextPdf", target)
        self._preserve_stage_pdf(2, "source-text-on-base", target)
        self.set_artifact("sourceBuildReport", report_path)

    def step_11_translate_segments(self) -> None:
        self._init_tm()
        payload = read_json(self.state_dir / "segments.json", {}) or {}
    ...
    def step_12_shape_text(self) -> None:
    ...
    def step_13_layout_text(self) -> None:
    ...
    def step_14_build_translated_pdf(self) -> None:
        target = self.pdf_dir / "translated.pdf"
        layout_path = self.state_dir / "positioned-layout.json"
        report = self._build_text_layer_pdf(target, layout_path, "14_build_translated_pdf")
        report_path = self.state_dir / "translated-build-report.json"
        write_json(report_path, {"status": "ok", **report, "recordedAt": now_iso()})
        self.set_artifact("translatedPdf", target)
        self._preserve_stage_pdf(3, "translated-text-on-base", target)
        self.set_artifact("translatedBuildReport", report_path)

    def step_15_validate_output_pdf(self) -> None:
        translated_pdf = self.pdf_dir / "translated.pdf"
        if command_path("qpdf"):
            result = run_command(["qpdf", "--check", str(translated_pdf)], timeout=120)
            report = {"tool": "qpdf", "status": "ok" if result["exitCode"] == 0 else "failed", "result": result}
            if result["exitCode"] != 0:
                write_json(self.state_dir / "output-validation.json", report)
                raise StepError("15_validate_output_pdf", "Output QPDF validation failed")
        elif module_available("pikepdf") and self.config.allow_degraded:
            import pikepdf  # type: ignore
            with pikepdf.open(translated_pdf) as document:
                report = {"tool": "pikepdf-degraded", "status": "degraded", "pageCount": len(document.pages), "objectCount": len(document.objects)}
        elif module_available("fitz") and self.config.allow_degraded:
            import fitz  # type: ignore
            with fitz.open(translated_pdf) as document:
                report = {"tool": "pymupdf-degraded", "status": "degraded", "pageCount": document.page_count}
        else:
            raise StepError("15_validate_output_pdf", "No output validation tool is available")
        path = self.state_dir / "output-validation.json"
        write_json(path, report)
        self.set_artifact("outputValidation", path)

    def step_16_render_diff(self) -> None:
        translated_pdf = self.pdf_dir / "translated.pdf"
        output_render = self._render_pdf(translated_pdf, self.pages_dir / "output", "output")
        ...
        build_report = read_json(self.state_dir / "translated-build-report.json", {}) or {}
        quality_status = "ok"
        reasons = []
    ...
    def step_17_publish_output(self) -> None:
        translated_pdf = self.pdf_dir / "translated.pdf"
        output_path = self._make_output_path()
        self._copy_file(translated_pdf, output_path)
...
def run_doctor(config: Config) -> int:
    report = tool_report()
    required_commands = ["qpdf", "sqlite3"]
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="run-v12.sh")
