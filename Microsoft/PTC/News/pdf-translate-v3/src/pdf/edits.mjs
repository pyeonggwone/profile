...
export function addTextBoxEmbedded({ page, x, y, width, height, text, fontPath, fontName, size = DEFAULT_FONT_SIZE, color = DEFAULT_COLOR }) {
    if (!fontPath) {
    ...
    return {
        ...
        text: String(text || ''),
        fontPath: String(fontPath),
        fontName: String(fontName || 'PDFTrFont'),
        size: Number(size) || DEFAULT_FONT_SIZE,
        color: Array.isArray(color) && color.length === 3
            ? color.map((c) => Number(c) || 0)
            : DEFAULT_COLOR,
