...
Preserve every placeholder matching __EPUBSTR_*_0000__ EXACTLY (do not translate, alter, reorder, or remove them).
Do not translate URLs, emails, variables, or protected glossary terms (they are placeholders).
...
export async function translateBatch(items, cfg, glossaryRows) {
    ...
    const system = buildSystem(cfg, glossaryRows);
    const userLines = [];
    items.forEach((it, i) => {
        userLines.push(`=== ${i + 1} ===`);
        userLines.push(it.text);
    });
    ...
    if (!parsed) {
        throw new Error('LLM 응답 === N === 블록 파싱 실패');
    }
    return parsed;
}
