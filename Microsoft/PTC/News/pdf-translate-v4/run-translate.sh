...
if [ -z "$PDF_ENGINE_FROM_ENV" ] && [ -f .env ]; then
  PDF_ENGINE_FROM_ENV="$(grep -E '^PDF_ENGINE=' .env 2>/dev/null | tail -1 | cut -d '=' -f2- || true)"
fi
...
if [ -z "$PDF_ENGINE_BIN_FROM_ENV" ] && [ -f .env ]; then
  PDF_ENGINE_BIN_FROM_ENV="$(grep -E '^PDF_ENGINE_BIN=' .env 2>/dev/null | tail -1 | cut -d '=' -f2- || true)"
fi
