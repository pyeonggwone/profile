...
- **읽기 전용 마인드**: APPLY 는 항상 v1 의 incremental update 로 새 PDF 를 만들고, 원본은 변경하지 않는다.
...
- DRM / 암호 PDF 자동 우회 없음 (v1 의 `from_bytes` 가 거부하면 EXTRACT 단계에서 실패).
