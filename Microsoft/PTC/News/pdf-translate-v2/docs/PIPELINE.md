...
- v1 의 `pdf_analysis::extract` 가 ToUnicode CMap 까지 적용한 텍스트 런을 반환.
...
- 번역된 segment 에 대해 `EditOperation::AddText` 객체를 생성:
    ...
    - `font` = `Helvetica` (Base14, v1 제약)
    - `size` = 원본 run 의 font_size
- 결과: `work/<stem>/edits.json`.
- `pdftr edit <input> <work>/edits.json <output>` 호출.
- v1 의 `IncrementalWriter` 가 원본 PDF 의 byte prefix 를 그대로 두고 incremental update section 을 append.
