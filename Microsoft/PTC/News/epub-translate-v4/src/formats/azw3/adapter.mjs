...
export async function read(filePath) {
    ...
    const warnings = ['AZW3 native writer는 MVP 초안입니다. 현재 단계에서는 구조 분석 후 안전하게 skip합니다.'];
    if (isLikelyDrmProtected(palm.buffer)) {
        return { skipped: true, reason: 'DRM protected', metadata: { title: palm.name }, warnings };
    }
    return {
        skipped: true,
        reason: 'AZW3 native translation writer is not implemented yet',
...
export async function write() {
    throw new Error('AZW3 writer is not implemented yet');
}
