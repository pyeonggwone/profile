...
export function loadGlossary(filePath) {
    ...
    for (const record of records) {
        ...
        const isProtected = String(record.protected || '').trim().toLowerCase() === 'true';
...
export function glossaryPrompt(rows) {
    ...
    return rows
        .map((row) => {
            const suffix = row.protected ? ' (protected, keep exactly)' : '';
            return `- ${row.term} => ${row.translation || row.term}${suffix}`;
        })
        .join('\n');
}
