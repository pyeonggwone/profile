...
`pdf-translate-v5` 는 원본 PDF를 번역 PDF로 재생성하기 위한 품질 우선 설계 프로젝트다.

v5는 하나의 PDF 엔진으로 모든 기능을 처리하지 않는다. PDF 처리 기능을 단계별로 나누고, 각 기능에 가장 적합한 도구를 연결한다. 구현 난이도와 프로젝트 복잡성은 평가 기준에서 제외한다.
...
- 원본 PDF 위에 whiteout/overlay 방식으로 덮어쓰지 않는다.
- 새 PDF를 생성한다.
...
| 단일 실행 진입점 | 사용자는 `run-v5.sh` 하나로 전체 파이프라인을 실행한다. |
...
./run-v5.sh bootstrap
./run-v5.sh input/sample.pdf
./run-v5.sh --strict input/sample.pdf
./run-v5.sh --translation-mode openai input/sample.pdf
./run-v5.sh --translation-mode azure-openai input/sample.pdf
./run-v5.sh --ocr local input/sample.pdf
./run-v5.sh --ocr azure input/sample.pdf
```
...
이 디렉토리는 v5 2차 connector 구현까지 진행된 상태다. 전체 품질 목표 엔진이 모두 연결된 완성본은 아니며, 설치되지 않은 고급 도구는 `degraded`, `skipped`, `missing-tools` 상태로 기록한다.
...
- `run-v5.sh`
- `src/pdf_translate_v5/pipeline.py`
- `.env.example`
- `requirements.txt`
- `package.json`
