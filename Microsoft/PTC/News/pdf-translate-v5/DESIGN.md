# DESIGN.md — pdf-translate-v5 디자인

이 문서는 `pdf-translate-v5` 의 시스템 디자인을 정의한다.

v5는 원본 PDF를 번역 PDF로 재생성하는 품질 우선 파이프라인이다. 모든 기능을 하나의 PDF 엔진에 몰아넣지 않고, 기능별 최적 도구를 연결한다.
...
`run-v5.sh` 는 orchestration만 담당한다. PDF 처리 로직은 각 단계 도구/script가 담당한다.

## 주요 컴포넌트

| 컴포넌트 | 책임 |
|---|---|
| `run-v5.sh` | 전체 단계 실행, 상태 갱신, 실패 처리 |
...
v5는 단일 DB에 모든 것을 넣지 않는다. 데이터 성격에 맞는 상태 파일을 사용한다.
