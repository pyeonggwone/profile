# EXECUTION_PLAN.md — pdf-translate-v5 실행계획

이 문서는 `pdf-translate-v5` 를 실제 구현할 때의 실행계획을 정의한다.

목표는 원본 PDF를 번역 PDF로 만드는 것이다. 구현 난이도는 고려하지 않고, 기능별 최적 도구를 연결한다.

## 실행 원칙

- 사용자는 `run-v5.sh` 하나만 실행한다.
- `run-v5.sh` 는 orchestrator다.
...
v5 실행계획이 완료되었다고 보는 기준:

- `run-v5.sh input/source.pdf` 로 전체 pipeline 실행 가능
...
v5 실행계획은 단일 엔진 구현이 아니라, 기능별 최적 도구를 `.sh` 파이프라인으로 연결하는 방식이다. 핵심은 각 단계의 언어가 아니라 공통 상태 schema와 산출물 계약이다.
