...
def _format_input_block(batch: list[dict]) -> tuple[str, str]:
    ...
    for idx, seg in enumerate(batch, start=1):
        ...
        if cat == "title":
            if sentence_form:
                rules.append(
                    f"  #{idx} TITLE (sentence): preserve sentence form, natural Korean. "
                ...
                rules.append(
                    ...
                    f"≤ {limit} Korean chars. NO trailing punctuation. "
                    "Forbidden endings: 다/요/니다/합니다/입니다/하세요/되세요/하십시오/하기."
                )
        elif cat == "subtitle":
            if sentence_form:
                rules.append(
                    f"  #{idx} SUBTITLE (sentence): preserve sentence form, natural Korean."
                )
            else:
                limit = min(max(int(len(src) * 1.1), 6), _TITLE_MAX_CHARS + 5)
                rules.append(
                    f"  #{idx} SUBTITLE: concise phrase. ≤ {limit} Korean chars. "
                    "Prefer noun ending. No trailing period."
                )

    examples = (
        "Examples (title noun-phrase conversion):\n"
        "  EN: Get started today  →  KO: 지금 시작\n"
        "  EN: Stop legacy        →  KO: 레거시 중단\n"
        "  EN: Modernize your apps → KO: 앱 현대화\n"
        "  EN: Why Azure?         →  KO: Azure 선택 이유\n"
    )

    system = (
        f"You translate {settings.source_lang} to {settings.target_lang} for slide decks.\n"
    ...
    for seg in segments:
        ...
        if cached is not None and not _is_corrupted(cached):
            ...
            if cached is not None and _is_corrupted(cached):
                try:
                    with _tm_connect() as conn:
                        conn.execute("DELETE FROM tm WHERE src_hash=?", (_hash(cleaned_for_llm),))
...
def _apply_paragraph_text(text_frame, para_index: int, kind: str, new_text: str,
                          runs_meta: list[dict] | None, kr_font: str) -> None:
    ...
    if kind == "tf2":
        ...
        try:
            ...
            paras.Text = new_text
            try:
                paras.Font.Name.NameComplexScript = kr_font
            except com_error:
                pass
            try:
                paras.Font.NameFarEast = kr_font
            except com_error:
                pass
            try:
                paras.Font.Name = kr_font
            except com_error:
                pass
        except com_error:
            pass
    ...
    try:
        ...
        applied.Font.Name = kr_font
    except com_error:
        pass
    try:
        applied.Font.NameFarEast = kr_font
    except com_error:
        pass
    ...
    if runs_meta:
        ...
        for m in runs_meta:
            ...
            try:
                ...
                continue
            _font_apply(ch.Font, m["font"], kr_font=kr_font)
            if "hyperlink" in m:
            ...
            for item in items:
                ...
                try:
                    ...
                    _apply_paragraph_text(
                        ...
                        item.get("runs_meta"), settings.kr_font,
                    )
