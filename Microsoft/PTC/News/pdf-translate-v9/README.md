...
원본 포맷 유지를 위해 번역 입력에는 text run별 `layoutLimit`을 포함한다. font size 10 기준 visual unit으로 영어, 한글, CJK, 공백, 기호 폭을 계산하고, 실제 font size와 horizontal scaling을 반영한다. 긴 공백과 char/word spacing은 정렬 여백일 수 있으므로 `LAYOUT_SPACING_CREDIT_RATIO`만큼만 번역 가능 폭으로 인정한다. 번역 후 visual width가 초과되면 `translation-report.json`에 `LAYOUT_VISUAL_WIDTH_OVERFLOW`로 기록한다.

layout fit 기본값:

```dotenv
LAYOUT_FIT_ENABLED=true
LAYOUT_FIT_MODE=report-only
LAYOUT_DEFAULT_FONT_SIZE=10
LAYOUT_SAFETY_RATIO=0.90
LAYOUT_WEIGHT_ASCII=0.55
LAYOUT_WEIGHT_HANGUL=1.00
LAYOUT_WEIGHT_CJK=1.00
LAYOUT_WEIGHT_SPACE=0.28
LAYOUT_WEIGHT_REPEATED_SPACE=0.12
